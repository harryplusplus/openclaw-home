...
def check_format(paths: list[str], fix: bool, config_path: str) -> dict:
...
def check_lint(paths: list[str], fix: bool, config_path: str) -> dict:
    """Run ruff lint check or fix."""
    cmd = ["uvx", "ruff", "check", "--config", config_path, "--output-format", "json"]
    if fix:
        cmd.append("--fix")
    cmd.extend(paths)
...
def check_types(paths: list[str], project_dir: str) -> dict:
    """Run pyright type check."""
    cmd = ["uvx", "--with", "types-PyYAML", "pyright", "--project", project_dir, "--outputjson"]
    cmd.extend(paths)
...
def main() -> None:
    ...
    parser.add_argument("--format-only", action="store_true", help="Only check formatting")
    parser.add_argument("--lint-only", action="store_true", help="Only run lint checks")
    parser.add_argument("--typecheck-only", action="store_true", help="Only run type checks")
    ...
    if run_typecheck:
        ...
        # We dynamically add extraPaths so pyright can resolve sibling imports (e.g. _common)
        with tempfile.TemporaryDirectory() as tmpdir:
            pyright_dir = Path(tmpdir) / "project"
            pyright_dir.mkdir()
            base_config = json.loads(pyright_config.read_text(encoding="utf-8"))
            # Collect unique directories containing the target .py files
            script_dirs = list({str(Path(p).resolve().parent) for p in args.paths if Path(p).is_file()})
            if not script_dirs:
                # If paths are directories, add them directly
                script_dirs = [str(Path(p).resolve()) for p in args.paths]
            base_config["extraPaths"] = list(dict.fromkeys(base_config.get("extraPaths", []) + script_dirs))
