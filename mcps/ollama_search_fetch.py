...
try:
    ...
    _FASTMCP_AVAILABLE = False  # type: ignore
...
def _web_search_impl(query: str, max_results: int = 3) -> dict[str, Any]:
    res = client.web_search(query=query, max_results=max_results)  # type: ignore
    return res.model_dump()  # type: ignore


def _web_fetch_impl(url: str) -> dict[str, Any]:
    res = client.web_fetch(url=url)  # type: ignore
    return res.model_dump()  # type: ignore


if _FASTMCP_AVAILABLE:
    app = FastMCP("ollama-search-fetch")  # type: ignore

    @app.tool()
    def web_search(query: str, max_results: int = 3) -> dict[str, Any]:  # type: ignore
        """
    ...
    @app.tool()
    def web_fetch(url: str) -> dict[str, Any]:  # type: ignore
        """
